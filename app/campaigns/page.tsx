...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        setAuthToken(stored);
        console.log("[CampaignsPage] Auth token loaded from localStorage");
      } else {
        setAuthToken(undefined);
        console.log("[CampaignsPage] No auth token found");
      }
    } catch (e) {
      console.error("[CampaignsPage] Error reading auth token:", e);
    }
  }, [user]);
  ...
  const fetchDeletedCampaigns = async () => {
    try {
      ...
      console.error("[CampaignsPage] Error fetching deleted campaigns:", err);
    }
  };
  ...
  useEffect(() => {
    console.log(
      "[CampaignsPage] Fetched campaigns updated:",
      fetchedCampaigns
    );
  ...
  const convertLegacyCampaign = (legacy: LegacyCampaign): Campaign => {
    console.log("[CampaignsPage] Converting legacy campaign:", legacy);
    const converted = {
      id: legacy.id,
    ...
    console.log("[CampaignsPage] Converted to:", converted);
    return converted;
  };
  ...
  useEffect(() => {
    console.log(
      "[CampaignsPage] Filtering campaigns. Status filter:",
      statusFilter,
      "Campaigns:",
      campaigns
    );
    ...
    console.log("[CampaignsPage] Filtered campaigns:", filtered);
  ...
  const displayCampaigns = useMemo(() => {
    console.log(
      "[CampaignsPage] Computing displayCampaigns. useSearch:",
      useSearch,
      "filteredCampaigns:",
      filteredCampaigns
    );
    if (useSearch) {
      console.log("[CampaignsPage] Using search results:", searchedCampaigns);
    ...
    const converted = filteredCampaigns.map((c) => {
      ...
      if ((c as LegacyCampaign).job_role !== undefined) {
        console.log("[CampaignsPage] Converting legacy campaign:", c);
        return convertLegacyCampaign(c as LegacyCampaign);
      }
      // Otherwise it's already a full Campaign object
      console.log("[CampaignsPage] Using existing campaign:", c);
      return c as Campaign;
    });
    console.log("[CampaignsPage] Converted display campaigns:", converted);
    return converted;
  ...
  const deleteCampaign = async (id: string) => {
    ...
    try {
      ...
      console.error("[CampaignsPage] Error deleting campaign:", err);
  ...
  const restoreCampaign = async (id: string) => {
    ...
    try {
      ...
      console.error("[CampaignsPage] Error restoring campaign:", err);
    } finally {
  ...
  const permanentlyDeleteCampaign = async (id: string) => {
    ...
    try {
      ...
      console.error(
        "[CampaignsPage] Error permanently deleting campaign:",
        err
      );
    } finally {
  ...
  // Debug logging
  useEffect(() => {
    console.log("[CampaignsPage] State debug:", {
